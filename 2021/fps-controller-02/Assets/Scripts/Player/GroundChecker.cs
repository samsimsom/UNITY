...
public class GroundChecker : MonoBehaviour {
    ...
    public void CheckGround(Vector3 origin)
    {
        ...
        if (Physics.SphereCast(origin, sphereCastRadius, Vector3.down, 
            out hit, sphereCastDistance, castingMask))
        {
    ...
    void OnDrawGizmos()
    {
        if (showDebug)
        {
            ...
            Vector3 startPoint = new Vector3(transform.position.x, 
                transform.position.y - (_controller.height / 2) + startDistanceFromBottom, 
                transform.position.z);
            Vector3 endPoint = new Vector3(transform.position.x, 
                transform.position.y - (_controller.height / 2) + startDistanceFromBottom - sphereCastDistance, 
                transform.position.z);
